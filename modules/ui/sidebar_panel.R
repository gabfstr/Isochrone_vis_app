...
sidebar_panel = function() {
  wellPanel(id = 'controls',
            ...
            input_ui(input_id = 'origin',
                     ...
                     help = paste0('Center of the isochrone request. The Isochrones will cover all roads which can be reached from ',
            ...
            input_ui(input_id = 'departure',
                     input_element = airDatepickerInput('departure', label = 'Departure date:', width = '100%',
                                                        value = Sys.Date()),
                     help = paste0('Date when travel is expected to start. Traffic speed, public transport schedules and incidents are taken into ',
            ...
            input_ui(input_id = 'time',
                     input_element = sliderInput('time', 'Time range: ', min = 0, max = 23, post = ':00', step = 1,
                                                 value = 17),
                     help = paste0('Time when travel is expected to start. Traffic speed, public transport schedules and incidents are taken into ',
                                   'account when calculating the route. Departure time can be past, present or future.'),
                     margin = 18),
            # mode #########################################################################################################
            ...
            div(style = inline_block_css(),
                ...
                column(1, style = 'margin-top: 4px; margin-left: 12px;',
                       ...
                       bsPopover('range_help', placement = 'right', trigger = 'click', title = NULL,
                                 content = 'Range of the isochrones. The unit is minutes.')
                )
            ),
            # step #########################################################################################################
            input_ui(input_id = 'step',
                     input_element = numericInput('step', label = 'Interval size:', min = 1, step = 1, value = 5),
                     help = paste0('Interval size between each isochrone : the unit ',
                                   'is minutes.')),
